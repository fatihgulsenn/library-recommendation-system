        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-3">
            {isAuthenticated && user ? (
              <>
                <span className="text-slate-700 font-semibold px-3 py-2 rounded-lg bg-white/70 border border-white/60">
                  {user.name}
                </span>
                <button
                  type="button"
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-white/20 animate-slide-in">
            ...
            <div className="mt-4 space-y-2">
              {isAuthenticated && user ? (
                <>
                  <div className="text-slate-700 font-semibold py-2 px-4 rounded-lg bg-white/70 border border-white/60">
                    {user.name}
                  </div>
                  <button
                    type="button"
                    onClick={logout}
                    className="block w-full text-left text-slate-700 hover:text-violet-600 transition-colors py-2 px-4 rounded-lg hover:bg-violet-50 font-semibold"
                  >
                    Logout
                  </button>
                </>
